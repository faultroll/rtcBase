...
{
#if defined(WEBRTC_WIN)
    *evnt = CreateEvent(NULL,  // Security attributes.
                        manual_reset, initially_signaled,
                        NULL);  // Name.
    return ((*evnt) == NULL) ? -1 : 0;
#else
    evnt->is_manual_reset_ = manual_reset;
    evnt->event_status_ = initially_signaled;
    Rtc_MtxInit(&evnt->event_mutex_);
    return pthread_cond_init(&evnt->event_cond_, NULL);
#endif
}
...
void Rtc_EvntDestroy(Evnt *evnt)
{
    ...
    CloseHandle(evnt);
#else
    Rtc_MtxDestroy(&evnt->event_mutex_);
    pthread_cond_destroy(&evnt->event_cond_);
#endif
}
...
void Rtc_EvntSet(Evnt *evnt)
{
    ...
    SetEvent(evnt);
#else
    Rtc_MtxLock(&evnt->event_mutex_);
    evnt->event_status_ = true;
    pthread_cond_broadcast(&evnt->event_cond_);
    Rtc_MtxUnlock(&evnt->event_mutex_);
#endif
}
...
{
#if defined(WEBRTC_WIN)
    return (WaitForSingleObject(evnt, milliseconds) == WAIT_OBJECT_0);
#else
    int error = 0;
    ...
    Rtc_MtxLock(&evnt->event_mutex_);
    if (milliseconds != INFINITE) {
        while (!evnt->event_status_ && error == 0) {
            error = pthread_cond_timedwait(&evnt->event_cond_, &evnt->event_mutex_, &ts);
        }
    } else {
        while (!evnt->event_status_ && error == 0)
            error = pthread_cond_wait(&evnt->event_cond_, &evnt->event_mutex_);
    }

    // NOTE(liulk): Exactly one thread will auto-reset this event. All
    // the other threads will think it's unsignaled.  This seems to be
    // consistent with auto-reset events in WEBRTC_WIN
    if (error == 0 && !evnt->is_manual_reset_)
        evnt->event_status_ = false;
