...
// eg. float r = rtc::SafeMin<float>(a,b);
template <typename R, typename T1, typename T2>
constexpr R SafeMin(T1 a, T2 b) {
  return SafeLt(a, b) ? static_cast<R>(a) : static_cast<R>(b);
}
// eg. float r = rtc::SafeMax<float>(a,b);
template <typename R, typename T1, typename T2>
constexpr R SafeMax(T1 a, T2 b) {
  return SafeGt(a, b) ? static_cast<R>(a) : static_cast<R>(b);
}
#endif /* SafeMin SafeMax */
...
#else /* SafeClamp */
// eg. float r = rtc::SafeClamp<float>(a,b,c);
template <typename R, typename T, typename L, typename H>
R SafeClamp(T x, L min, H max) {
  RTC_DCHECK_LE(min, max);
  return SafeLe(x, min)
             ? static_cast<R>(min)
             : SafeGe(x, max) ? static_cast<R>(max) : static_cast<R>(x);
}
#endif /* SafeClamp */
