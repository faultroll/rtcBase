...
int32_t TraceImpl::AddThreadId(char* trace_message) const {
  uint32_t thread_id = (uint32_t)rtc::ThrdCurrent();
  // Messages is 12 characters.
  return sprintf(trace_message, "%10u; ", thread_id);
}

#if defined(WEBRTC_WIN)
int32_t TraceImpl::AddTime(char* trace_message,
...
int32_t TraceImpl::AddDateTimeInfo(char* trace_message) const {
  // tick
  /* struct timespec ts;
  timespec_get_systime(&ts);
...
int32_t TraceImpl::AddTime(char* trace_message, const TraceLevel level) const {
  // tick
  struct timespec ts;
  timespec_get_systime(&ts);
  int64_t dw_current_time = timespec_to_millisec(&ts);
