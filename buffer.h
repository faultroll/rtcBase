 ...
 private:
  ...
  void MaybeZeroCompleteBuffer() {
    if (ZeroOnFree && capacity_ > 0) {
      ...
      memset(data_.get(), 0, capacity_ * sizeof(T)); // ExplicitZeroMemory(data_.get(), capacity_ * sizeof(T));
    }
  }
  ...
  void ZeroTrailingData(size_t count) {
    ...
    memset(data_.get() + size_, 0, count * sizeof(T)); // ExplicitZeroMemory(data_.get() + size_, count * sizeof(T));
  }
