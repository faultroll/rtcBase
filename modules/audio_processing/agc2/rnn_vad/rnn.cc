...
std::vector<float> GetPreprocessedFcWeights(
  ...
  const size_t input_size = /* rtc::CheckedDivExact */RTC_CHECK_DIV_EXACT(weights.size(), output_size);
...
std::vector<float> GetPreprocessedGruTensor(
  ...
  const size_t n =
      /* rtc::CheckedDivExact */RTC_CHECK_DIV_EXACT(tensor_src.size(), output_size * kNumGruGates);
