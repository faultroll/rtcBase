...
void DownSampler::Initialize(int sample_rate_hz) {
  RTC_DCHECK(
      sample_rate_hz == AudioProcessing::kSampleRate8kHz || sample_rate_hz == AudioProcessing::kSampleRate16kHz ||
      sample_rate_hz == AudioProcessing::kSampleRate32kHz || sample_rate_hz == AudioProcessing::kSampleRate48kHz);

  sample_rate_hz_ = sample_rate_hz;
  down_sampling_factor_ = /* rtc::CheckedDivExact */RTC_CHECK_DIV_EXACT(sample_rate_hz_, 8000);
...
void DownSampler::DownSample(rtc::ArrayView<const float> in,
  ...
  const size_t kMaxNumFrames = AudioProcessing::kSampleRate48kHz * AudioProcessing::kChunkSizeMs / 1000;
  ...
  data_dumper_->DumpWav("lc_down_sampler_output", out, AudioProcessing::kSampleRate8kHz, 1);
}
