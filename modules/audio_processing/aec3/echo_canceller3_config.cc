...
bool Limit(float* value, float min, float max) {
  float clamped = rtc::SafeClamp<float>(*value, min, max);
...
bool Limit(size_t* value, size_t min, size_t max) {
  size_t clamped = rtc::SafeClamp<size_t>(*value, min, max);
  bool res = *value == clamped;
  *value = clamped;
  return res;
}
...
bool Limit(int* value, int min, int max) {
  int clamped = rtc::SafeClamp<int>(*value, min, max);
  bool res = *value == clamped;
  *value = clamped;
  return res;
}
...
EchoCanceller3Config::Delay::Delay() {}
EchoCanceller3Config::Delay::Delay(
    const EchoCanceller3Config::Delay& e) = default;
EchoCanceller3Config::Delay& EchoCanceller3Config::Delay::operator=(
    const Delay& e) = default;

/* EchoCanceller3Config::Mask::Mask() {}
EchoCanceller3Config::Mask::Mask(const EchoCanceller3Config::Mask& m) {} */
...
EchoCanceller3Config::Suppressor::Tuning::operator=(
    const Tuning& e) = default;
