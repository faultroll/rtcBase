    ...
    for (size_t k = 0; k < kFftLengthBy2; ++k) {
      ...
      float correction_factor =
          correction_factors_[ch][n_active_sections_[ch][k]]
                             [band_to_subband_[k]];
      erle_[ch][k] = rtc::SafeClamp<float>(average_erle[ch][k] * correction_factor,
  ...
  for (size_t ch = 0; ch < converged_filters.size(); ++ch) {
    if (converged_filters[ch]) {
      ...
      for (size_t subband = 0; subband < kSubbands; ++subband) {
        ...
        erle_estimators_[ch][idx][subband] = rtc::SafeClamp<float>(
      ...
      for (size_t subband = 0; subband < kSubbands; ++subband) {
        ...
        erle_ref_[ch][subband] = rtc::SafeClamp<float>(erle_ref_[ch][subband],
