...
bool UseMaxAnalogChannelLevel() {
  return true/* field_trial::IsEnabled("WebRTC-UseMaxAnalogAgcChannelLevel") */;
}
...
int ClampLevel(int mic_level, int min_mic_level) {
  return rtc::SafeClamp<int>(mic_level, min_mic_level, kMaxMicLevel);
}
...
MonoAgc::MonoAgc(ApmDataDumper* data_dumper,
  ...
  /* if (use_agc2_level_estimation) {
    ...
    agc_ = std::unique_ptr<Agc>(new Agc());
  }
}
...
void MonoAgc::Process(const int16_t* audio,
                      size_t samples_per_channel,
                      int sample_rate_hz) {
  new_compression_to_set_ = rtc::nullopt;
...
void MonoAgc::UpdateGain() {
  ...
  int raw_compression =
      rtc::SafeClamp<int>(rms_error, kMinCompressionGain, max_compression_gain_);
  ...
  const int residual_gain =
      rtc::SafeClamp<int>(rms_error - raw_compression, -kMaxResidualGainChange,
...
AgcManagerDirect::AgcManagerDirect(int num_capture_channels,
  ...
  for (size_t ch = 0; ch < channel_agcs_.size(); ++ch) {
    ...
    channel_agcs_[ch] = std::unique_ptr<MonoAgc>(new MonoAgc(
...
void AgcManagerDirect::AnalyzePreProcess(const AudioBuffer* audio) {
  RTC_DCHECK(audio);
  AnalyzePreProcess(audio->channels_const_f(), audio->num_frames());
}
...
rtc::Optional<int> AgcManagerDirect::GetDigitalComressionGain() {
