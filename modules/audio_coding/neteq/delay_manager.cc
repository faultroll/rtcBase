...
std::unique_ptr<DelayManager> DelayManager::Create(
  ...
  return std::unique_ptr<DelayManager>(new DelayManager(
      max_packets_in_buffer, kMinBaseMinimumDelayMs, quantile, mode,
      peak_detector, tick_timer, std::move(histogram)));
}
...
int DelayManager::Update(uint16_t sequence_number,
  ...
  // (liuguangyuan) |enable_rtx_handling_| decides 
  // whether |DelayManager| should handle retransmitted packets
